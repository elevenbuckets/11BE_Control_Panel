...
const appOpts = require('../ControlPanel.json');
...
ipcMain.on('reload', (e, args) => {
	...
	let cwd = process.cwd();
	cwd = path.join(cwd, "../../..");
	exec(path.join(cwd, 'node_modules', '.bin', 'bladecli') + ' ControlPanel', { cwd: cwd, env: { DISPLAY: process.env.DISPLAY, XAUTHORITY: process.env.XAUTHORITY, PATH: process.env.PATH } });
	app.exit();
});
